...
#| eval: false
#| include: false
gp3 <- p + geom_line(aes(color="Individual Players"), alpha=0.15, show.legend = TRUE) + 
...
nall <- readRDS("nall.rds")

# Global ACF across all players
acf_vals <- readRDS("acf_vals.rds")
...
#| include: false
diagnostic_plots <- plot(check_model(fit_mixed, size_title=16, size_axis_title = 12,panels=FALSE))
```
...
career_seq <- readRDS("car_seq.rds")
...
ggplotly(p2, tooltip = "text") %>%
  layout(
    autosize = TRUE,
    title = list(
      text = "<b>Fixed Effects on Points per Game</b>",
      x = 0.5,
      xanchor = "center",
      y = 0.95,
      yanchor = "top",
    ...
    margin = list(l = 80, r = 20, t = 60, b = 50)  # balanced bottom
  ) %>%
  ...
  htmlwidgets::onRender("
    function(el, x) {
      function resizeLayout() {
        ...
        var height = isMobile ? window.innerHeight * 0.75 : window.innerHeight * 0.55;

        // Font sizes
        var titleSize = isMobile ? 12 : 16;
        var axisLabelSize = isMobile ? 6 : 11;
        var axisTitleSize = isMobile ? 10 : 14;
        var legendTitleSize = isMobile ? 10 : 12;
        var legendTextSize = isMobile ? 8 : 10;

        // Y-axis tick angle
        var yTickAngle = isMobile ? -45 : 0;

        // Margins
        var marginTop = isMobile ? 70 : 60;
        var marginBottom = isMobile ? 80 : 70;  // keep small, like p3
        var marginLeft = 80;
        var marginRight = isMobile ? 60 : 20;

        // Legend
        var legendOrientation = isMobile ? 'h' : 'v';
        var legendX = isMobile ? 0.5 : 1.02;
        var legendY = isMobile ? -0.2 : 1;
        var legendXanchor = isMobile ? 'center' : 'left';
        var legendYanchor = 'top';
        ...
        Plotly.relayout(el, {
          ...
          'yaxis.tickfont.size': axisLabelSize,
          'yaxis.tickangle': yTickAngle,
